    ...
    return (
        <>
            <div className='cantidadProductos'>
                <div className='pointer' onClick={clickLess}>
                    -
                </div>
                <div>{counterItem}</div>
                <div className='pointer' onClick={clickPlus}>
                    +
                </div>
            </div>
            {isInCart(props.id) ? (
                <div className='buttonCart'>
                    <Link className='linkCart hoverLink' to='/Cart'>
                        Ir al Carrito
                    </Link>
                </div>
            ) : (
                <button
                    type='button'
                    className='btn btn-dark buttonFinish'
                    onClick={() => {
                        props.onAdd(counterItem);
                    }}
                >
                    <span>Enviar al Cart</span>
                </button>
            )}
        </>
    );
